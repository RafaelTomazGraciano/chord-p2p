...
async function handleNodeRequest(node, request, response) {
  try {
    ...
    if (request.method === 'PUT' && url.pathname === '/rpc/files') {
      ...
      // O dono replica para a própria successor-list assim que grava a cópia primária.
      await node.replicateToSuccessors(body.name, content);
    ...
    if (request.method === 'PUT' && url.pathname === '/rpc/replicate') {
      const body = await readJson(request);
      const content = Buffer.from(body.content || '', 'base64');
      await node.storeReplica(body.ownerId, body.name, content);
      return json(response, 200, { ok: true, size: content.length });
    }
    if (request.method === 'DELETE' && url.pathname === '/rpc/replicate') {
      const body = await readJson(request);
      await node.deleteReplica(body.ownerId, body.name);
      return json(response, 200, { ok: true });
    }
    if (request.method === 'GET' && url.pathname === '/rpc/replica') {
